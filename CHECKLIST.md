...
- ✅ JWT access token (short-lived, in memory) + httpOnly refresh cookie, with rotation and stolen-token reuse detection (reuse nukes all sessions for that user)
...
- ✅ Real-time send/receive over Socket.IO (JWT-authenticated sockets)
...
- ⬜ A dedicated game-client API key/bot-token auth path (distinct from browser cookie auth) for your VR game to authenticate as a lightweight client — currently the VR game would need to do the same JWT login flow as the website. Worth a follow-up if the game engine can't easily manage httpOnly cookies.

...
import { useAppStore, type Message, type Member } from "../../store/app";
...
export default function MessageList({ channelId, guildId, onEdit, onReply }: Props) {
  ...
  const mentionContext = {
    resolveUser: (id: string) => memberMap[id]?.user.username ?? undefined,
    resolveRole: (id: string) => roleMap[id]?.name,
    currentUserId: currentUser.id,
  };

  const typingNames = [...(typing ?? [])].map((id) => memberMap[id]?.user.username ?? "Someone");

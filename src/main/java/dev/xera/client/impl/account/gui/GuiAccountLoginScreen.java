package dev.xera.client.impl.account.gui;
...
import dev.xera.client.impl.account.Account;
...
public class GuiAccountLoginScreen extends GuiScreen {
    ...
    public void drawScreen(int par1, int par2, float par3) {
        ...
        mc.fontRenderer.drawStringWithShadow("Accounts: " + XeraClient.getInstance().getAccountManager().getAlts().size(), 2, 2, 0xA0A0A0);
    }
    ...
    protected void actionPerformed(GuiButton p_146284_1_) {
        ...
        if (p_146284_1_.id == 1) {
            ...
            try {
                Account account = XeraClient.getInstance().getAccountManager().getAlts().get(slots.selected);
                XeraClient.getInstance().getAccountManager().login(account);
            } catch (IndexOutOfBoundsException ignored) {
            }
        } else if (p_146284_1_.id == 3) {

            if (slots.selected != -1 && !XeraClient.getInstance().getAccountManager().getAlts().isEmpty()) {
                Account account = XeraClient.getInstance().getAccountManager().getAlts().get(slots.selected);
            ...
            if (random.enabled) {
                List<Account> accounts = XeraClient.getInstance().getAccountManager().getAlts();
                int rand = MathUtils.random(0, accounts.size() - 1);

                slots.selected = rand;
                XeraClient.getInstance().getAccountManager().login(accounts.get(rand));
            }
    ...
    protected void keyTyped(char par1, int par2) {
        ...
        slots.selected = MathHelper.clamp_int(slots.selected, 0, XeraClient.getInstance().getAccountManager().getAlts().size() - 1);
    }
    ...
    public void updateScreen() {
        ...
        if (random != null) {
            random.enabled = !XeraClient.getInstance().getAccountManager().getAlts().isEmpty();
        }
    }
    ...
    public void confirmClicked(boolean par1, int par2) {
        if (par1) {
            switch (par2) {
                ...
                case 10: {
                    XeraClient.getInstance().getAccountManager().getAlts().clear();
                ...
                case 20: {
                    try {
                        Account account = XeraClient.getInstance().getAccountManager().getAlts().get(slots.selected);
                        XeraClient.getInstance().getAccountManager().remove(account);
                    } catch (IndexOutOfBoundsException ignored) {
                    }
                    break;
                }

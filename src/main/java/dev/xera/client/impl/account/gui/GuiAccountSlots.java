...
import dev.xera.client.impl.account.AccountType;
...
public class GuiAccountSlots extends GuiSlot implements Wrapper {
    ...
    protected int getSize() {
        return XeraClient.getInstance().getAccountManager().getAlts().size();
    }

    @Override
    protected void elementClicked(int var1, boolean var2, int var3, int var4) {
        if (var2) {
            Account account = XeraClient.getInstance().getAccountManager().getAlts().get(var1);

...
import dev.xera.client.impl.module.combat.Aura;
...
public class HUD extends Module {
    public static String WATERMARK = XeraClient.NAME;
    ...
    public void onRenderHUD(EventRender2D event) {
        ...
        if (lag.getValue()) {
            ...
            if (!XeraClient.getInstance().getTickManager().responding) {
                double time = (System.currentTimeMillis() - XeraClient.getInstance().getTickManager().lastTick) / 1000.0;
        ...
        if (watermark.getValue()) {
            mc.fontRenderer.drawStringWithShadow(
                    WATERMARK + " " + XeraClient.VERSION.getVersionString(),
                    3, 3,
                    Colors.rainbow.getValue() ? Colors.getClientRainbow(0) : Colors.getColorInt());
        }

        if (arraylist.getValue()) {
            List<ToggleableModule> modules = XeraClient.getInstance()
        ...
        if (tps.getValue()) {
            double t = XeraClient.getInstance().getTickManager().getTps();

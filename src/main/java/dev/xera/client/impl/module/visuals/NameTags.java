...
import dev.xera.client.impl.event.impl.render.EventRenderVanillaNameTag;
    ...
    public void onRenderWorld(EventRender3D event) {
        if (mc.renderViewEntity != null) {
            for (EntityPlayer player : mc.theWorld.playerEntities) {
                ...
                if (player.equals(mc.thePlayer) && !XeraClient.getInstance().getModuleManager().getModule(FreeCamera.class).isRunning()) {
                    continue;
                }
...
//        glDisable(GL_BLEND);
    ...
    private String getDisplayInfo(EntityPlayer player) {
        ...
        if (XeraClient.getInstance().getFriendManager().isFriend(player) || player.equals(mc.thePlayer)) {

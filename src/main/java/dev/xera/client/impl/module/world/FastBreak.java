...
import dev.xera.client.impl.event.impl.player.EventUpdate;
import dev.xera.client.impl.event.impl.render.EventRender3D;
import dev.xera.client.impl.event.impl.world.EventClickBlock;
...
import dev.xera.client.utils.render.renderers.impl.three.OutlinedBox;
...
public class FastBreak extends ToggleableModule {
        ...
        if (sentBreak) {
            if (WorldUtils.isReplaceable(x, y, z)) {
                ...
                if (progress >= 0.94) {
                    ...
                    if (autoSwitch.getValue() && slot != -1 && XeraClient.getInstance().getInventoryManager().serverSlot != slot) {
    ...
    public void onClickBlock(EventClickBlock event) {
        ...
        slot = XeraClient.getInstance().getInventoryManager().serverSlot;

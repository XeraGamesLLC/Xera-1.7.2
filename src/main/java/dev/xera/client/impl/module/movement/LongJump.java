...
public class LongJump extends ToggleableModule {
    ...
    public void onTick(EventTick event) {
        ...
        if (damage.getValue().equals(Damage.BOW) && !damaged) {
            ...
            XeraClient.getInstance().getRotationManager().setRotations(new float[] { mc.thePlayer.rotationYaw, -90.0f });

            if (pullBackTicks < pullTicks.getValue()) {
                mc.thePlayer.sendQueue.addToSendQueue(new C08PacketPlayerBlockPlacement(-1, -1, -1, 255, XeraClient.getInstance().getInventoryManager().getHeld(), 0.0f, 0.0f, 0.0f));
                ++pullBackTicks;
            } else {

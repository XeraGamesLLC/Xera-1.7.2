...
public class AutoTunnel extends ToggleableModule {
    ...
    public void onTick(EventTick event) {
        ...
        if (state == 0) {
            ...
            if (swap.getValue().equals(Swap.NONE) && XeraClient.getInstance().getInventoryManager().serverSlot != slot) {
                return;
            }
...
//            }
            ...
            if (!fillPositions.isEmpty()) {
                ...
                for (EnumFacing facing : EnumFacing.values()) {
                    ...
                    boolean needsSwap = XeraClient.getInstance().getInventoryManager().serverSlot != slot;
                    if (needsSwap) {
                        switch (swap.getValue()) {
                            case SERVER: {
                                mc.thePlayer.sendQueue.addToSendQueue(new C09PacketHeldItemChange(slot));

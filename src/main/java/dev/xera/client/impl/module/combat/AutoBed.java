...
import dev.xera.client.impl.event.impl.render.EventRender3D;
import dev.xera.client.impl.module.ModuleCategory;
import dev.xera.client.impl.module.ToggleableModule;
import dev.xera.client.impl.module.active.Colors;
import dev.xera.client.impl.module.visuals.LogoutSpots;
import dev.xera.client.impl.module.world.FreeCamera;
...
import dev.xera.client.utils.render.RenderEngine;
import dev.xera.client.utils.world.DamageUtils;
...
public class AutoBed extends ToggleableModule {
    ...
    public void onTick(EventTick event) {
        ...
        if (rotate.getValue().equals(Rotate.NORMAL)) {
            ...
            XeraClient.getInstance().getRotationManager().setRotations(RotationUtils.calcAngles(rotate));
        } else if (rotate.getValue().equals(Rotate.DIRECTIONAL)) {
            float yaw = MathHelper.wrapAngleTo180_float(info.facing.hIndex * 90.0f);
            XeraClient.getInstance().getRotationManager().setRotations(new float[] { yaw, mc.thePlayer.rotationPitch });
        }
                ...
                if (event.getPacket() instanceof S23PacketBlockChange) {
                    ...
                    if (packet.func_148880_c() != null && packet.func_148880_c().equals(Blocks.bed)) {
                        ...
                        if ((int) info.vec3.xCoord == x || (int) info.vec3.yCoord == y || (int) info.vec3.zCoord == z) {
                            mc.thePlayer.sendQueue.addToSendQueue(new C08PacketPlayerBlockPlacement(x, y, z, EnumFacing.UP.order_a, XeraClient.getInstance().getInventoryManager().getHeld(), 0.5f, 0.5f, 0.5f));
                            ...
                            if (slot != -1) {
                                swapToBed(slot);
                                mc.thePlayer.sendQueue.addToSendQueue(new C08PacketPlayerBlockPlacement(x, y - 1, z, EnumFacing.UP.order_a, XeraClient.getInstance().getInventoryManager().getHeld(), 0.5f, 0.5f, 0.5f));
    ...
    private void swapToBed(int slot) {
        switch (swap.getValue()) {
            ...
            case SERVER: {
                if (XeraClient.getInstance().getInventoryManager().serverSlot != slot) {
                    swapped = true;
    ...
    private float damageAtForCurrentTarget(Vec3 vec) {
        ...
        if (protectFriends.getValue()) {
            for (EntityPlayer player : new ArrayList<>(mc.theWorld.playerEntities)) {
                ...
                if (!XeraClient.getInstance().getFriendManager().isFriend(player)) {
                    continue;
                }
    ...
    private boolean isValidTarget(EntityPlayer e) {
        ...
        if (XeraClient.getInstance().getFriendManager().isFriend(e)) {
            return false;
        }

        // don't attack our camera guy
        if (XeraClient.getInstance().getModuleManager().getModule(FreeCamera.class).isRunning() && e.getEntityId() == -133769420) {
            return false;
        }

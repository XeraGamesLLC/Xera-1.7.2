...
import dev.xera.client.utils.render.renderers.impl.three.FilledBox;
            ...
            if (explode.getValue() && !await.getValue()) {
                ...
                if (entityTarget != -1 && explodeTimer.getTimePassedMs() / 50.0 >= 20.0 - explodeSpeed.getValue()) {
                    if (rotate.getValue() && event.getEra().equals(Era.PRE)) {
                        XeraClient.getInstance().getRotationManager().setRotations(RotationUtils.calcAngles(placePos.addVector(0.5, 0.0, 0.5).offset(EnumFacing.UP)));
                        return;
                    }
            ...
            if (place.getValue()) {
                switch (swap.getValue()) {
                    case PACKET: {
                        ...
                        if (XeraClient.getInstance().getInventoryManager().serverSlot != crystalSlot) {
                ...
                if (placeTimer.getTimePassedMs() / 50.0 >= 20.0 - placeSpeed.getValue() && canPlaceAt(placePos)) {
                    ...
                    if (rotate.getValue() && event.getEra().equals(Era.PRE)) {
                        XeraClient.getInstance().getRotationManager().setRotations(RotationUtils.calcAngles(placePos.addVector(0.5, 0.0, 0.5).offset(EnumFacing.DOWN)));
                        return;
                    }
                    ...
                    if (result) {
                        ...
                        if (swap.getValue().equals(Swap.PACKET)) {
                            XeraClient.getInstance().getInventoryManager().sync();
                        }
                    }

                    // mc.thePlayer.sendQueue.addToSendQueue(new C08PacketPlayerBlockPlacement(px, py - 1, pz, EnumFacing.UP.order_a, XeraClient.getInstance().getInventoryManager().getHeld(), 0.5f, 0.5f, 0.5f));
            ...
            if (event.getPacket() instanceof S0EPacketSpawnObject) {
                ...
                if (packet.getType() == 51 && placePos != null) {
                    ...
                    if (placePos.xCoord == x && placePos.yCoord == y && placePos.zCoord == z) {
                        ...
                        if (await.getValue()) {
                            if (rotate.getValue()) {
                                XeraClient.getInstance().getRotationManager().setRotations(RotationUtils.calcAngles(placePos.offset(EnumFacing.UP)));
                                return;
                            }
...
//                            mc.theWorld.removePlayerEntityDangerously(entity);
    ...
    private boolean checkTargetValidity(EntityPlayer t) {
        return t != null
                ...
                && t.getHealth() > 0.0f
                && !XeraClient.getInstance().getFriendManager().isFriend(t.getCommandSenderName())

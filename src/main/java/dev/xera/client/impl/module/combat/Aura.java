...
import dev.xera.client.utils.world.EntityUtils;
...
public class Aura extends ToggleableModule {
    ...
    public void onTick(EventMotionUpdate event) {
        ...
        if (rotate.getValue()) {
            ...
            XeraClient.getInstance().getRotationManager().setRotations(angles);
        }
    ...
    private boolean isValidTarget(EntityLivingBase e) {
        ...
        if (e instanceof EntityPlayer && XeraClient.getInstance().getFriendManager().isFriend((EntityPlayer) e)) {
            return false;
        }

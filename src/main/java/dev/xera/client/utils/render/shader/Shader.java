package dev.xera.client.utils.render.shader;
...
public abstract class Shader {
    ...
    public Shader(String frag) {
        this(frag, "/assets/minecraft/xera/shaders/vertex.vsh");
    }
    ...
    public Shader(String frag, String vert) {
        ...
        try {
            ...
            if (fragShader == 0 || vertShader == 0) {
                XeraClient.LOGGER.error("Failed to compile shaders!");
                return;
            }
        }
        ...
        if (ARBShaderObjects.glGetObjectParameteriARB(program, ARBShaderObjects.GL_OBJECT_LINK_STATUS_ARB) == GL_FALSE) {
            XeraClient.LOGGER.error(getLogInfo(program));
            return;
        }
